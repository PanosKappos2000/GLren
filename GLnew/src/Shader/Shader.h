...
public:
	...
	void SetUniformMatrix4D(const char* varName, const glm::mat4& var)
	{
		glUniformMatrix4fv(glGetUniformLocation(m_tag, varName), 1, GL_FALSE, glm::value_ptr(var));
	}

	void SetUniformMatrix3D(const char* varName, const glm::mat3& var)
	{
		glUniformMatrix3fv(glGetUniformLocation(m_tag, varName), 1, GL_FALSE, glm::value_ptr(var));
	}

	void SetUniformVector3D(const char* varName, const glm::vec3& var)
	{
		glUniform3fv(glGetUniformLocation(m_tag, varName), 1, glm::value_ptr(var));
	}

	void SetUniformFloat(const char* varName, float var)
	{
		glUniform1f(glGetUniformLocation(m_tag, varName), var);
	}

	void SetUniformInt(const char* varName, int var)
	{
		glUniform1i(glGetUniformLocation(m_tag, varName), var);
	}

	void SetUniformUnsignedInt(const char* varName, unsigned int var)
	{
